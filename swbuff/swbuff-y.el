...
;;
;; - Checks for selected buffer in other windows.  RET has if it is found
;; the special meaning of displaying that frame.  In this case the
;; buffer name is underlined.
;; 
;; - "n" means display selected buffer in new frame.
;;
;; - These commands are also available in a popup menu (assigned to
;; <apps>).
;;
...
;; on/off with the minor mode swbuff-y-mode. The purpose of this is to
;; provide more easy testing and cooperation custom themes.
...
(define-key swbuff-y-keymap [(?m)]
  (lambda()(interactive) (message "bind=%s, emul2=%s" (key-binding [(?m)]) emulation-mode-map-alists)))
;;(global-set-key [(apps)] 'swbuff-y-show-popup)
  ...
  (when (eq (selected-frame) swbuff-initial-frame)
    ...
    (unless (or (eq this-command 'swbuff-kill-this-buffer)
		...
		(eq this-command 'swbuff-switch-to-next-buffer)
                (eq this-command 'swbuff-y-maybe-display-other-frame)
                (eq this-command 'swbuff-y-show-in-new-frame)
                (eq this-command 'swbuff-y-show-popup)
...
`swbuff-post-command-hook'"
  ...
  (if (boundp 'emulation-mode-map-alists)
    ...
    (setq minor-mode-map-alist (delq 'swbuff-y-keymap minor-mode-map-alist)))
...
the delay specified by `swbuff-clear-delay'."
  ...
  (if (boundp 'emulation-mode-map-alists)
    ...
    (unless (memq 'swbuff-y-keymap minor-mode-map-alist)
      (setq  minor-mode-map-alist (cons 'swbuff-y-keymap minor-mode-map-alist))))
